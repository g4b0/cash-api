...
- **Dashboard** (`GET /balance/@communityId/@memberId`): reads the member's balance (computed at runtime). Authorization: user must be in the same community as the requested member.
- **Transactions list** (`GET /transactions/@communityId/@memberId(/@num(/@page))`): returns paginated list of merged income and expense transactions, sorted by date descending. Pagination: `num` (1-100, default 25), `page` (≥1, default 1). Authorization: user must be in the same community as the requested member.
